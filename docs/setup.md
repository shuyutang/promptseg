...
| Disk | ~310 MB of weights, fetched from the Hugging Face Hub the first time the server starts |
...
so the image does not have to be rebuilt to change models.
...
| Everything disappeared after a restart | State is in memory. Export before stopping the server |
